...
def get_i2_file(root_dir, temp):
    """
    Finds the msd xvg file for a given temperature in the i2 directory.
    ...
    pattern = os.path.join(root_dir, f"{temp_num}K")
    ...
    # We need subdirectory "i2"
    for d in dirs:
        i2_dir = os.path.join(d, "i2")
        if os.path.isdir(i2_dir):
            # Look for xvg file
            # Naming convention: msd_mmc_300K.xvg
            # Check for any msd_mmc_*.xvg
            xvg_pattern = os.path.join(i2_dir, "msd_mmc_*.xvg")
...
def plot_arrhenius_i2(csv_files):
    """
    ...
    if grom_data:
        ...
        ax1.set_title('Arrhenius Plot (i2 Runs): GROMACS D')
    ...
    ax2.set_title('Arrhenius Plot (i2 Runs): Calculated D')
    ...
    plt.tight_layout()
    output_path = 'arrhenius_plots_i2.png'
...
def main():
    root_dir = os.getcwd()
    # Temperatures from 300 to 370, step 5
    temps_int = list(range(300, 375, 5))
    temps = [f"{t}K" for t in temps_int]
    
    generated_csvs = []
    
    print("Starting Post-Processing for i2 directories...")
    
    for temp in temps:
        xvg_file = get_i2_file(root_dir, temp)
        ...
        if xvg_file:
            ...
            try:
                # IMPORTANT: analyze_msd_log.analyze_and_plot might save files to the same dir as xvg_file
                # so it should work fine for i2 without changes if it uses dirname(xvg_file)
            ...
            print(f"Warning: No i2 file found for {temp}")
            
    # Run Arrhenius Plot
    if generated_csvs:
        plot_arrhenius_i2(generated_csvs)
    else:
